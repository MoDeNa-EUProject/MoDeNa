...
os.system(command1)
#mypath12=r'/home/mohammad/spherepack/'
...
for i in range(0,27):
    for j in range(0,NumOfCells):
        fff.write('{0:f}\n'.format(Rads[j]))
fff.close()
commandTessellation="neper -T -n {0:d} -domain 'cube(14,14,14)' -morpho @Centers.txt -weight @Rads.txt -o RVE27 -format geo -statcell vol -statedge length -statface area -statver x".format((27*NumOfCells))
os.system(commandTessellation)
################################################################
######Extraction of middle Representative volume element########
################################################################
#mypath12=r'C:\Users\Mohammad\Desktop'
