__author__ = 'imdea-Mohammad Marvi Mashhadi: mohammad.marvi@imdea.org'
import os
import os.path
...
PUinWall=0.60          #Amount of material in walls (in percentage %)
############################################################
...
for i in range(0,27):
    for j in range(0,NumOfCells):
        fff.write('{0:f}\n'.format(Rads[j]))
fff.close()
commandTessellation="neper -T -n {0:d} -domain 'cube(14,14,14)' -morpho @Centers.txt -weight @Rads.txt -regularization 1 -mloop 15 -o RVE27 -format geo -statcell vol -statedge length -statface area -statver x".format((27*NumOfCells))
