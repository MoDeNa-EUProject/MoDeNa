...
CONTAINS
  ...
  SUBROUTINE lmdif1(fcn, m, n, x, fvec, tol, epsfcn, factor, info, iwa)
    ...
    REAL :: ftol, gtol, xtol, fjac(m,n)
    !REAL, PARAMETER :: factor = 100._dp, zero = 0.0_dp
  ...
  SUBROUTINE lmdif(fcn, m, n, x, fvec, ftol, xtol, gtol, maxfev, epsfcn,  &
    ...
    REAL, INTENT(OUT)     :: fjac(:,:)    ! fjac(ldfjac,n)
    ...
    REAL, PARAMETER :: one = 1.0, p1 = 0.1, p5 = 0.5,  &
         p25 = 0.25, p75 = 0.75, p0001 = 0.0001, &
         zero = 0.0
...
300 IF (iflag < 0) info = iflag
  ...
  SUBROUTINE lmder1(fcn, m, n, x, fvec, fjac, tol, info, ipvt)
    ...
    REAL, INTENT(IN OUT)  :: x(:)
    REAL, INTENT(OUT)     :: fvec(:)
    REAL, INTENT(IN OUT)  :: fjac(:,:)    ! fjac(ldfjac,n)
    REAL, INTENT(IN)      :: tol
    INTEGER, INTENT(OUT)       :: info
...
300 IF (iflag < 0) info = iflag
  ...
  SUBROUTINE lmpar(n, r, ipvt, diag, qtb, delta, par, x, sdiag)
    ...
    REAL :: wa1(n), wa2(n)
    REAL, PARAMETER :: p1 = 0.1, p001 = 0.001, zero = 0.0
...
220 IF (iter == 0) par = zero
  ...
  SUBROUTINE qrfac(m, n, a, pivot, ipvt, rdiag, acnorm)
    ...
    INTEGER, INTENT(OUT)       :: ipvt(:)
    REAL, INTENT(OUT)     :: rdiag(:)
    REAL, INTENT(OUT)     :: acnorm(:)
    ...
    REAL :: ajnorm, epsmch, sum, temp, wa(n)
...
40     ajnorm = enorm(m-j+1, a(j:,j))
  ...
  SUBROUTINE qrsolv(n, r, ipvt, diag, qtb, x, sdiag)
    ...
    REAL :: COS, cotan, qtbpj, SIN, sum, TAN, temp, wa(n)
    REAL, PARAMETER :: p5 = 0.5, p25 = 0.25, zero = 0.0
  ...
  FUNCTION enorm(n,x) RESULT(fn_val)
    ...
    REAL, PARAMETER :: one = 1.0, zero = 0.0, rdwarf = 3.834E-20,  &
         rgiant = 1.304E+19
