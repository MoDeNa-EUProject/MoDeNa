...
REAL, INTENT(IN)           :: rhobeg
REAL, INTENT(IN)           :: rhoend
...
REAL  :: xbase(n), xopt(n), xnew(n), xpt(npt,n), pq(npt-1)
REAL  :: pl(npt,npt-1), h(n,n), g(n), d(n), vlag(npt), w(npt)
...
REAL  :: half = 0.5_dp, one = 1.0_dp, tol = 0.01_dp, two = 2.0_dp
REAL  :: zero = 0.0_dp
REAL  :: ddknew, delta, detrat, diff, distest, dnorm, errtol, estim
REAL  :: evalue, f, fbase, fopt, fsave, ratio, rho, rhosq, sixthm
REAL  :: sum, sumg, sumh, temp, tempa, tworsq, vmax, vquad, wmult
...
REAL  :: gg(n), td(n), tn(n), w(n), piv(n), z(n)

REAL  :: delsq, dhd, dnorm, dsq, dtg, dtz, gam, gnorm, gsq, hnorm
REAL  :: par, parl, parlest, paru, paruest, phi, phil, phiu, pivksv
REAL  :: pivot, posdef, scale, shfmax, shfmin, shift, slope, sum
REAL  :: tdmin, temp, tempa, tempb, wsq, wwsq, wz, zsq
INTEGER    :: i, iterc, j, jp, k, kp, kpp, ksav, ksave, nm
REAL  :: one = 1.0_dp, two = 2.0_dp, zero = 0.0_dp
...
REAL  :: half = 0.5_dp, one = 1.0_dp, zero = 0.0_dp
REAL  :: dd, dhd, dlin, dsq, gd, gg, ghg, gnorm, halfrt, hmax, ratio
REAL  :: scale, sum, sumv, temp, tempa, tempb, tempc, tempd, tempv
REAL  :: vhg, vhv, vhw, vlin, vmu, vnorm, vsq, vv, wcos, whw, wsin, wsq
INTEGER    :: i, j, k
