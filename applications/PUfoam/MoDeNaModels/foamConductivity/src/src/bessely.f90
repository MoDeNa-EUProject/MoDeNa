...
!  real*8 zr, zi
!  real*8 cyr(10), cyi(10), cwr(10), cwi(10)

!  n=5
!  zr=1.d0; zi=2.d0

!  call ZBESY(zr,zi,0.d0,1,n,cyr,cyi,nz,cwr,cwi,ierr)
...
!     COMPLEX CWRK,CY,C1,C2,EX,HCI,Z
      ...
      DIMENSION CYR(1), CYI(1), CWRKR(1), CWRKI(1)
...
!-----------------------------------------------------------------------
      ELIM = 2.303D0*(DBLE(FLOAT(K))*D1MACH(5)-3.0D0)
      EXR = DCOS(ZR)
      EXI = DSIN(ZR)
      EY = 0.0D0
      TAY = DABS(ZI+ZI)
   ...
   70 CONTINUE
      ...
      DO 80 I=1,N
        ...
        CYR(I) = -STI*HCII
        CYI(I) = STR*HCII
        IF (STR.EQ.0.0D0 .AND. STI.EQ.0.0D0 .AND. EY.EQ.0.0D0) NZ = NZ + 1
   80 CONTINUE
      RETURN
   90 CONTINUE
      C1R = EXR
