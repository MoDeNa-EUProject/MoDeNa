...
def saturated_pressure(name, temp):
    """
    Saturated pressure of gas. Implemented for cyclopentane and carbon dioxide.
...
def surface_tension(name, temp):
    ...
    return par[0] + par[1] * temp + par[2] * temp**2


def kelvin_effect(pres, surft, temp, mw_ba, dcell):
    """Saturated vapour pressure changes with curvature. It is assumed that cells are
    sperical. https://en.wikipedia.org/wiki/Kelvin_equation"""
...
def main():
    ...
    rhop = 1100  # polymer density
    mw_ba = {  # molecular weight
        'CyP': 72e-3,
        'CO2': 44e-3,
        'H2O': 18e-3
    }
    ...
    # volume_bag = 3e-3  # volume of tedlar bag
    ...
    for name in names:
        with open('cell_gas_{0}.csv'.format(name), 'w') as csvfile:
            ...
            for temp in temps:
                ...
                # c_ba = (w_ba_g + w_ba_c) * m_foam / volume_bag
                # print('Concentration of BA in bag {0:.3g} g/l'.format(c_ba))
                # p_ba_bag = c_ba * gas_constant * temp
                # print(
                #     'Partial pressure of BA in bag {0:.0f} Pa'.format(p_ba_bag))
    print('End.')
