...
if plots[2]: #not to scale when bubble grows
    ...
    surf = ax.plot_surface(time3d, radius3d*1e6, data*1e6, rstride=1,
        cstride=1, cmap=cm.coolwarm, linewidth=0, antialiased=False)
    # surf = ax.plot_surface(time3d, radius3d, data, rstride=2,
    #    cstride=2, cmap=cm.coolwarm, linewidth=0, antialiased=False)
    # ax.set_xlabel('time')
