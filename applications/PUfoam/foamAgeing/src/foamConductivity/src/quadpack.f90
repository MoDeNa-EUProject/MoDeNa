!partially adatped to use higher precision, but I'm not sure if the results
!are more than single-precision (frequent use of numerical costants)
module quadpack
...
!  Parameters:
!
!    Output, real(dp) PI, the value of pi.
!
  implicit none
!
  real(dp) pi
!
  pi = 3.14159265358979323846264338327950288419716939937510E+00_dp
...
!
  ...
  call qk15i ( f, boun, inf, 0.0e+00_dp, 1.0e+00_dp, result, abserr, defabs, resabs )
!
!  Test on accuracy.
!
  last = 1
...
!
  ...
  integer numrl2
  real(dp) omega
  real(dp), parameter :: p = 0.9E+00
  real(dp), parameter :: pi = 3.1415926535897932E+00
  real(dp) p1
  real(dp) psum(52)
  real(dp) reseps
  real(dp) result
...
!
  ...
  if ( omega == 0.0e+00 ) then
    ...
    if ( integr == 1 ) then
      call qagi ( f, 0.0e+00_dp, 1, epsabs, 0.0e+00_dp, result, abserr, neval, ier )
    else
      result = 0.0E+00
      abserr = 0.0E+00
      neval = 0
...
!
    ...
    call qfour ( f, c1, c2, omega, integr, epsa, 0.0e+00_dp, limit, lst, maxp1, &
...
!*******************************************************************************
!
!! R_SWAP swaps two real(dp) values.
!
!
!  Modified:
!
